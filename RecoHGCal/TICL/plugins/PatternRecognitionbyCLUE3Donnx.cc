...
#include "RecoHGCal/TICL/plugins/PatternRecognitionbyCLUE3Donnx.h"
...
PatternRecognitionbyCLUE3Donnx<TILES>::PatternRecognitionbyCLUE3Donnx(const edm::ParameterSet &conf, edm::ConsumesCollector iC)
...
template <typename TILES>
void PatternRecognitionbyCLUE3Donnx<TILES>::dumpTiles(const TILES &tiles) const {
      ...
      for (int phi = 0; phi < nPhiBin; phi++) {
        ...
        if (!tiles[layer][offset + iphi].empty()) {
          if (this->algo_verbosity_ > VerbosityLevel::Advanced) {
            edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "Layer: " << layer << " ieta: " << ieta << " phi: " << phi
...
template <typename TILES>
void PatternRecognitionbyCLUE3Donnx<TILES>::dumpTracksters(const std::vector<std::pair<int, int>> &layerIdx2layerandSoa,
                                                       const int eventNumber,
                                                       const std::vector<Trackster> &tracksters) const {
  if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
    edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx")
...
template <typename TILES>
void PatternRecognitionbyCLUE3Donnx<TILES>::dumpClusters(const TILES &tiles,
                                                     const std::vector<std::pair<int, int>> &layerIdx2layerandSoa,
                                                     const int eventNumber) const {
  if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
    edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "[evt, lyr, Seed,      x,       y,       z, r/|z|,   eta,   phi, "
...
template <typename TILES>
void PatternRecognitionbyCLUE3Donnx<TILES>::makeTracksters(
  ...
  if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
    edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "New Event";
  }
  ...
  for (unsigned int i = 0; i < rhtools_.lastLayer(); ++i) {
    ...
    if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
      edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "Layer " << i << " located at Z: " << layersPosZ_.back();
    }
  }
  ...
  for (auto const &lc : input.layerClusters) {
    if (input.mask[layerIdx] == 0.) {
      if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
        edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "Skipping masked cluster: " << layerIdx;
      }
    ...
    if (invClsize == 1.) {
      ...
      if (rhtools_.isSilicon(detId)) {
        ...
        if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
          edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "Single cell cluster in silicon, rx: " << std::setw(5)
  ...
  if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
    edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "Reconstructed " << nTracksters << " tracksters" << std::endl;
  ...
  for (unsigned int layer = 0; layer < clusters_.size(); ++layer) {
    ...
    if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
      edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "Examining Layer: " << layer;
    }
    for (unsigned int lc = 0; lc < thisLayer.x.size(); ++lc) {
      if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
        edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "Trackster " << thisLayer.clusterIndex[lc];
      }
      if (thisLayer.clusterIndex[lc] >= 0) {
        if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
          edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << " adding lcIdx: " << thisLayer.layerClusterOriginalIdx[lc];
        }
  ...
  if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
    for (auto const &t : result) {
      edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "Barycenter: " << t.barycenter();
      edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "LCs: " << t.vertices().size();
      edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "Energy: " << t.raw_energy();
      edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "Regressed: " << t.regressed_energy();
    }
  }
  ...
  reset();
  if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
    edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << std::endl;
  }
}

template <typename TILES>
void PatternRecognitionbyCLUE3Donnx<TILES>::energyRegressionORpId(const std::vector<reco::CaloCluster> &layerClusters,
...
template <typename TILES>
void PatternRecognitionbyCLUE3Donnx<TILES>::calculateLocalDensity(
  ...
  for (unsigned int i = 0; i < numberOfClusters; i++) {
    ...
    for (int currentLayer = minLayer; currentLayer <= maxLayer; currentLayer++) {
      if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
        edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "RefLayer: " << layerId << " SoaIDX: " << i;
        edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "NextLayer: " << currentLayer;
      }
      const auto &tileOnLayer = tiles[currentLayer];
      bool onSameLayer = (currentLayer == layerId);
      if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
        edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "onSameLayer: " << onSameLayer;
      }
      ...
      if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
        edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "eta: " << clustersOnLayer.eta[i];
        edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "phi: " << clustersOnLayer.phi[i];
        edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "etaBinMin: " << etaBinMin << ", etaBinMax: " << etaBinMax;
        edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "phiBinMin: " << phiBinMin << ", phiBinMax: " << phiBinMax;
      }
      for (int ieta = etaBinMin; ieta <= etaBinMax; ++ieta) {
        auto offset = ieta * nPhiBin;
        if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
          edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "offset: " << offset;
        }
        for (int iphi_it = phiBinMin; iphi_it <= phiBinMax; ++iphi_it) {
          int iphi = ((iphi_it % nPhiBin + nPhiBin) % nPhiBin);
          if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
            edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "iphi: " << iphi;
            edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx")
          ...
          for (auto otherClusterIdx : tileOnLayer[offset + iphi]) {
            ...
            if ((layerandSoa.first == -1) && (layerandSoa.second == -1)) {
              if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
                edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "Skipping masked layerIdx " << otherClusterIdx;
              }
              continue;
            }
            ...
            if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
              ...
              edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "OtherEta: " << clustersLayer.eta[layerandSoa.second];
              edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "OtherPhi: " << clustersLayer.phi[layerandSoa.second];
            }
            ...
            if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
              edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "Distance[eta,phi]: "
              ...
              edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "Distance[cm]: " << (dist * clustersOnLayer.z[i]);
              edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx")
                  << "Energy Other:   " << clustersLayer.energy[layerandSoa.second];
              edm::LogVerbatim("PatternRecognitionbyCLUE3Donnx") << "Cluster radius: " << clustersOnLayer.radius[i];
            }
            if (reachable) {
...
template <typename TILES>
void PatternRecognitionbyCLUE3Donnx<TILES>::calculateDistanceToHigher(
    const TILES &tiles, const int layerId, const std::vector<std::pair<int, int>> &layerIdx2layerandSoa) {
  constexpr int nEtaBin = TILES::constants_type_t::nEtaBins;
  constexpr int nPhiBin = TILES::constants_type_t::nPhiBins;
  auto &clustersOnLayer = clusters_[layerId];
...
template <typename TILES>
int PatternRecognitionbyCLUE3Donnx<TILES>::findAndAssignTracksters(
...
template <typename TILES>
void PatternRecognitionbyCLUE3Donnx<TILES>::fillPSetDescription(edm::ParameterSetDescription &iDesc) {
...
template class ticl::PatternRecognitionbyCLUE3Donnx<TICLLayerTiles>;
template class ticl::PatternRecognitionbyCLUE3Donnx<TICLLayerTilesHFNose>;
